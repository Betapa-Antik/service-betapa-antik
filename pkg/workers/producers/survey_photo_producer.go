...
func PublishSurveyPhotoUploadAsync(p payload.PhotoUploadPayload) {
	go func() {
		if err := publishSurveyPhotoUpload(p); err != nil {
			log.Printf("[RABBIT] publish failed: %v", err)
		}
	}()
}

func publishSurveyPhotoUpload(p payload.PhotoUploadPayload) error {
