...
func PublishPetugasPhotoUpload(p payload.PhotoUploadPayload) error {
	...
	log.Printf("Published photo upload message for petugas %s with %d file(s) to folde %s", p.ID, len(p.Files), p.Folder)
	return nil
}
